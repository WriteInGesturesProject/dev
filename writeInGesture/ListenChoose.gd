...
#	Choose word to say
	...
	card2 = Card.new()
	card3 = Card.new()
	find_node("plate").add_child(card1)
	find_node("plate").add_child(card2)
	find_node("plate").add_child(card3)
	...
	find_node("marginPlate").add_constant_override("margin_top", find_node("Speak").rect_size.y + yRest/2)
#	if(index + 3 == myWords.size()):
	card1.setUpCard(myWords[index], Global.level, sizeCard)
	card2.setUpCard(myWords[index], Global.level, sizeCard)
	card3.setUpCard(myWords[index], Global.level, sizeCard)
#		for b in range(0,3):
