...
func _ready():
	...
	if(Ex == null):
		return
	var success = true
	for i in Global.try: # Check if the user tried every words
		if(!i):
			success = false
			break
	var total = Ex.getNbWords() # This is the total of word in the exercise 
	if(Global.play == 2): # This is the Listen & Choose Game
		success = true
		total /= 3 # The total of word is divided by 3
	elif(Global.play == 1 && total > 22): # We can only display 22 images on the GooseGame
		total = 22
	elif(Global.play == 3 && total > Global.max_cards): # We display only 12 images at max on the MemoryGame
		total = Global.max_cards
	var percent : float = float(Global.score) / total * 100 # This is the score in percentage
	var comment = ""
	if(percent < 50):
		comment = "Dommage, essaye encore !"
	else:
		comment = "Bravo !!"
	if(Global.play != 0): # The user didn't chose a training exercise
